...
hace un tiempo leí _Clean Code_ de _Robert C. Martin_ y me encontré con algo bastante interesante:
...
esto creo que es una de las principales ventajas con respecto a mantener interfaces limpias en sistemas con repentinos cambios, donde simplificamos todos los parámetros que deberíamos pasarle al servicio a uno solo, con un simple DTO simplifica la interfaz en un instante, (ojo, como todo en la programación ganamos en algo pero perdemos en otra, el cúmulo de parámetros lo seguimos teniendo, pero en un objeto de transferencia de datos es mucho más simple de mantener, ya que si las reglas cambian solamente debemos cambiar este objeto y no cambiar las interfaces de nuestra capa de aplicación)
...
`ShippingService`, ahora si recordamos lo que dice el tío bob en el libro de _Clean Code_ 
...
```
namespace Presentation\Http\Adapters\Shipping;
...
estos se encargan de iterar los datos a la forma que la API necesita exponer
